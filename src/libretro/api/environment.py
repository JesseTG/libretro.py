...
RETRO_ENVIRONMENT_GET_TARGET_SAMPLE_RATE = 81 | RETRO_ENVIRONMENT_EXPERIMENTAL
RETRO_ENVIRONMENT_GET_NETPLAY_CLIENT_INDEX = 82 | RETRO_ENVIRONMENT_EXPERIMENTAL
RETRO_ENVIRONMENT_EXEC_MEM_ALLOC = 83
RETRO_ENVIRONMENT_EXEC_MEM_FREE = 84
RETRO_ENVIRONMENT_GET_AUDIO_SAMPLE_BATCH_FLOAT = 85 | RETRO_ENVIRONMENT_EXPERIMENTAL
RETRO_ENVIRONMENT_GET_MEMORY_STATUS = 86 | RETRO_ENVIRONMENT_EXPERIMENTAL
RETRO_ENVIRONMENT_SET_SERIALIZATION_QUIRKS = 87
RETRO_ENVIRONMENT_SET_SERIALIZATION_QUIRKS_V1 = 44
RETRO_ENVIRONMENT_GET_SCREEN_10BPC_CAPABLE = 88 | RETRO_ENVIRONMENT_EXPERIMENTAL
RETRO_ENVIRONMENT_GET_HDR_PAPER_WHITE_NITS = 89 | RETRO_ENVIRONMENT_EXPERIMENTAL
RETRO_ENVIRONMENT_GET_HDR_EXPAND_GAMUT = 90 | RETRO_ENVIRONMENT_EXPERIMENTAL
RETRO_ENVIRONMENT_GET_HDR_OUTPUT_MODE = 91 | RETRO_ENVIRONMENT_EXPERIMENTAL
RETRO_ENVIRONMENT_GET_HDR_MAX_NITS = 92 | RETRO_ENVIRONMENT_EXPERIMENTAL
RETRO_ENVIRONMENT_GET_VFS_AUTHORIZED_LOCATIONS = 93 | RETRO_ENVIRONMENT_EXPERIMENTAL
...
class EnvironmentCall(IntEnum):
    ...
    """
    The identifier that :attr:`SET_HW_RENDER` carried
    while it was still an experimental part of the ABI.

    Not a distinct environment call:
    RetroArch handles this and :attr:`SET_HW_RENDER` in the same branch,
    and libretro.py routes both to the same handler
    so that cores built against the older ABI still work.
    ...
    SET_SERIALIZATION_QUIRKS_V1 = RETRO_ENVIRONMENT_SET_SERIALIZATION_QUIRKS_V1
    """
    The identifier that :attr:`SET_SERIALIZATION_QUIRKS` used before it was renumbered to 87,
    kept so that cores built against older copies of ``libretro.h`` still work.

    ``libretro.h`` moved this environment call off 44
    because that number was also used by :attr:`SET_HW_SHARED_CONTEXT`
    (as ``44 | RETRO_ENVIRONMENT_EXPERIMENTAL``, so the two values never actually collided).
    RetroArch itself no longer answers the old identifier,
    but libretro.py accepts both so that existing cores remain usable as test subjects.
    ...
    GET_TARGET_SAMPLE_RATE = RETRO_ENVIRONMENT_GET_TARGET_SAMPLE_RATE
    GET_NETPLAY_CLIENT_INDEX = RETRO_ENVIRONMENT_GET_NETPLAY_CLIENT_INDEX

    EXEC_MEM_ALLOC = RETRO_ENVIRONMENT_EXEC_MEM_ALLOC
    """
    .. seealso:: :class:`.retro_exec_mem_alloc`
    """

    EXEC_MEM_FREE = RETRO_ENVIRONMENT_EXEC_MEM_FREE
    """
    .. seealso:: :class:`.retro_exec_mem_free`
    """

    GET_AUDIO_SAMPLE_BATCH_FLOAT = RETRO_ENVIRONMENT_GET_AUDIO_SAMPLE_BATCH_FLOAT
    """
    .. seealso:: :class:`.retro_audio_sample_float_callback`
    """

    GET_MEMORY_STATUS = RETRO_ENVIRONMENT_GET_MEMORY_STATUS
    """
    .. seealso:: :class:`.retro_memory_status`
    """

    GET_SCREEN_10BPC_CAPABLE = RETRO_ENVIRONMENT_GET_SCREEN_10BPC_CAPABLE
    """
    .. seealso:: :attr:`.PixelFormat.XRGB2101010`
    """

    GET_HDR_PAPER_WHITE_NITS = RETRO_ENVIRONMENT_GET_HDR_PAPER_WHITE_NITS
    GET_HDR_EXPAND_GAMUT = RETRO_ENVIRONMENT_GET_HDR_EXPAND_GAMUT
    """
    .. seealso:: :class:`.HdrExpandGamut`
    """

    GET_HDR_OUTPUT_MODE = RETRO_ENVIRONMENT_GET_HDR_OUTPUT_MODE
    """
    .. seealso:: :class:`.HdrOutputMode`
    """

    GET_HDR_MAX_NITS = RETRO_ENVIRONMENT_GET_HDR_MAX_NITS

    GET_VFS_AUTHORIZED_LOCATIONS = RETRO_ENVIRONMENT_GET_VFS_AUTHORIZED_LOCATIONS
    """
    .. seealso:: :class:`.retro_vfs_authorized_locations`
    """
