...
- Add support for resolving the old `SET_HW_RENDER` envcall value.
...
- Keep the `retro_vfs_dir_handle` associated with a `StandardDirectoryHandle` alive,
  to prevent hard-to-find undefined behavior when iterating over a directory.
...
- Removed `boundary=EJECT` from `Key` and `AvEnableFlags`
  for consistency with other `IntFlag`s.
- Make `CompositeEnvironmentDriver` non-generic.
- Give `ConstantPowerDriver.__init__` a default argument
  representing a plugged-in host.
- Refactor `Session.__init__` to accept arguments
  matching the `with_` methods of `SessionBuilder`.
